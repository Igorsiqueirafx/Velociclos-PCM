...
export default function Error({
  ...
  useEffect(() => {
    logEvent('error_boundary', 'error', 'Unexpected error', { error: error.message, digest: error.digest })
  }, [error])
      ...
      <div className="relative z-10 w-full max-w-md mx-auto px-4">
        <div className="bg-[#2a2e39] border border-[#404857] rounded-2xl p-8 shadow-[0_0_40px_rgba(0,0,0,0.3)] text-center">
          ...
          <div className="flex gap-4 justify-center">
            <button
              onClick={reset}
              className="px-6 py-3 bg-gradient-to-r from-[#ffd700] to-[#ffeb3b] text-[#1e2329] font-bold rounded-lg shadow-lg hover:from-[#ffdd33] hover:to-[#ffd700] transition-all duration-200"
            >
              Tentar novamente
            </button>
            <Link
              href="/"
              className="px-6 py-3 border border-[#404857] text-[#dcdcdc] font-medium rounded-lg hover:bg-[#3a3e49] transition-all duration-200"
            >
              Voltar ao início
            </Link>
          </div>
